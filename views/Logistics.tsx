...
const Logistics: React.FC<LogisticsProps> = ({ vehicles, setVehicles }) => {
   ...
   * Periodically updates vehicle coordinates using vector projection.
   */
  useEffect(() => {
    let interval: number;
      ...
      interval = window.setInterval(() => {
        setVehicles(prev => prev.map(v => {
          if (v.status === 'In Transit' && v.location) {
            // 1. Natural Drift: Update heading and speed slightly
            const headingDrift = (Math.random() - 0.5) * 8; // Drift up to 4 degrees
            const currentHeading = ((v.heading || 0) + headingDrift + 360) % 360;
            
            const speedFluctuation = (Math.random() - 0.5) * 4; // Fluctuate speed
            const currentSpeed = Math.max(30, Math.min(80, (v.speed || 50) + speedFluctuation));

            // 2. Vector Projection: Calculate new coordinates
  ...
  const calculateETA = (vehicle: Vehicle) => {
    ...
    const distance = Math.sqrt(dLat * dLat + dLng * dLng) * 111; // Approx 111km per degree
  ...
  const getTripProgress = (vehicle: Vehicle) => {
    ...
    // For simulation, we assume total distance is 5 degrees (approx 550km)
    const dLat = vehicle.destination.lat - vehicle.location.lat;
    const dLng = vehicle.destination.lng - vehicle.location.lng;
    const currentDist = Math.sqrt(dLat * dLat + dLng * dLng);
    const totalDist = 5; // Assumed fixed total distance for progress bar
  ...
  // Calculate Map Bounds based on current fleet positions and Zoom level
  const mapBounds = useMemo(() => {
    ...
    if (followVehicle && selectedVehicle?.location) {
      ...
      latRange = 0.5; // Constant base range when following
      lngRange = 0.5;
    } else {
      // Focus on fleet average
    ...
    // Apply Zoom factor: smaller range = higher zoom
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center space-x-3">
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter">FLEET COMMAND</h2>
            <div className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase transition-colors ${
              ...
              <span>{isSimulating ? 'Tracking Active' : 'Satellite Standby'}</span>
            </div>
          </div>
          <p className="text-slate-500 text-sm mt-1">Digital logistics twin for DR Congo fuel distribution</p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span>{isSimulating ? 'STOP SIMULATION' : 'START LIVE GPS'}</span>
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 flex-1 min-h-[600px]">
        {/* Sidebar: Unit Selection, Telemetry & Manual Controls */}
        <div className="xl:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Deployment Roster</h3>
            <span className="text-[10px] font-bold text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200">{vehicles.length} Units</span>
          </div>
            ...
            <div className="bg-slate-900 text-white divide-y divide-slate-800">
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Digital Telemetry</span>
                  <div className="flex items-center space-x-2">
                    <button 
                    ...
                    >
                      {followVehicle ? 'LOCK ON' : 'FREE CAM'}
                    </button>
                  </div>
                </div>
                ...
                {selectedVehicle.destination && (
                  <div className="mb-4 bg-slate-800/30 p-3 rounded-xl border border-slate-800/50">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest">Active Destination</p>
                        <p className="text-xs font-bold text-amber-400">{selectedVehicle.destination.name}</p>
                      </div>
                      {calculateETA(selectedVehicle) && (
                        <div className="text-right">
                          <p className="text-[9px] text-emerald-500 uppercase font-black tracking-widest">ETA</p>
                    ...
                    <div className="mt-3">
                       <div className="flex justify-between items-center mb-1">
                         <span className="text-[8px] font-black text-slate-600 uppercase">Trip Progress</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-800">
                    <p className="text-[9px] text-slate-500 uppercase font-black mb-1.5 tracking-widest">Velocity</p>
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-800">
                    <p className="text-[9px] text-slate-500 uppercase font-black mb-1.5 tracking-widest">Bearing</p>
                    <p className="text-2xl font-black text-white">{Math.round(selectedVehicle.heading || 0)}° <span className="text-[10px] font-medium text-slate-400 uppercase text-amber-500">True</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 bg-slate-950/50">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">Manual Deployment Override</span>
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-amber-500 hover:bg-amber-600 text-slate-900 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-amber-500/10 active:scale-95"
                >
        ...
        <div className="xl:col-span-3 bg-slate-950 rounded-2xl relative overflow-hidden shadow-2xl border border-slate-800 flex items-center justify-center group/map">
          ...
          <div className="absolute inset-8 rounded-[2rem] border border-white/5 bg-slate-900/40 backdrop-blur-sm overflow-hidden shadow-inner">
             ...
             {/* Render Strategic Paths (Lines to Destination) */}
             ...
             {/* Render Vehicles as Interactive Tactical Nodes */}
             {vehicles.map((v) => v.location && (
               ...
               >
                 <div className="relative">
                   ...
                   {/* Orientation Indicator (Rotating arrow) */}
                   {v.status === 'In Transit' && (
                     <div 
                   ...
                   <div className={`relative w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all duration-500 ${
                     selectedVehicleId === v.id 
                       ? 'bg-amber-500 border-white shadow-[0_0_30px_rgba(245,158,11,0.6)]' 
                   ...
                   <div className={`absolute top-full mt-3 left-1/2 -translate-x-1/2 px-2.5 py-1 rounded-md text-[9px] font-black uppercase whitespace-nowrap border-2 transition-all shadow-xl flex flex-col items-center ${
                     selectedVehicleId === v.id 
                       ? 'bg-white text-slate-900 border-amber-500 opacity-100 translate-y-0' 
                       : 'bg-slate-900 text-slate-500 border-slate-800 opacity-0 group-hover/map:opacity-100 translate-y-1'
                   }`}>
                     <span className="font-bold">{v.plateNumber}</span>
                     {calculateETA(v) && (
                       <span className="text-emerald-600 border-t border-slate-100 mt-0.5 pt-0.5">ETA: {calculateETA(v)}</span>
                     )}
                   </div>
                 </div>
               </div>
             ...
             {/* Background Stylized Topography */}
          ...
          <div className="absolute top-12 left-12 flex flex-col space-y-2 pointer-events-auto z-30">
            <button 
              onClick={zoomIn}
              className="w-10 h-10 bg-black/60 backdrop-blur-xl border border-white/10 rounded-xl flex items-center justify-center text-white hover:bg-amber-500 hover:text-slate-900 transition-all shadow-xl active:scale-95 group"
              title="Zoom In"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="absolute top-12 right-12 flex flex-col items-end space-y-3 pointer-events-none">
            <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl">
              <div className="flex items-center space-x-4">
                <div className="text-right">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Active Corridor</p>
            ...
            <div className="bg-emerald-500/10 backdrop-blur-md border border-emerald-500/20 rounded-lg px-3 py-1.5 flex items-center space-x-2">
              ...
              <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-tighter">Satellite Uplink Optimized</span>
            </div>
          </div>
          ...
          <div className="absolute bottom-12 left-12 flex items-center space-x-5 bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl text-[10px] text-slate-400 font-mono pointer-events-none">
            <div className="space-y-1">
              <div className="flex items-center space-x-3">
                <span className="w-12 text-slate-600 font-black uppercase">Lat Min</span>
                <span className="text-slate-200">{mapBounds.minLat.toFixed(4)}</span>
              </div>
              <div className="flex items-center space-x-3">
                <span className="w-12 text-slate-600 font-black uppercase">Lat Max</span>
                <span className="text-slate-200">{mapBounds.maxLat.toFixed(4)}</span>
              </div>
            </div>
            <div className="w-px h-10 bg-white/10"></div>
            <div className="space-y-1">
              <div className="flex items-center space-x-3">
                <span className="w-12 text-slate-600 font-black uppercase">Lng Min</span>
                <span className="text-slate-200">{mapBounds.minLng.toFixed(4)}</span>
              </div>
              <div className="flex items-center space-x-3">
                <span className="text-slate-600 font-black uppercase w-12 text-right px-0">ZOOM</span>
                <span className="text-amber-500 font-bold">x{zoom.toFixed(1)}</span>
              </div>
            </div>
          </div>

          {/* Tactical Crosshair Legend */}
          <div className="absolute bottom-12 right-12">
            <div className="w-16 h-16 border border-white/10 rounded-full flex items-center justify-center relative opacity-30">
              <div className="absolute w-full h-px bg-white/20"></div>
              <div className="absolute h-full w-px bg-white/20"></div>
              <div className="absolute -top-4 text-[9px] font-black text-white tracking-widest">N</div>
              <div className="absolute -right-4 text-[9px] font-black text-white tracking-widest">E</div>
            </div>
          </div>
        </div>
      </div>
